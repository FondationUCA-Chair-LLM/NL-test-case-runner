...
async function extract(data: Obs,page: Page, ret?: z.AnyZodObject, terms?: string): Promise<string> {
  var result = "{\n";
  ...
  {  
    result2 =  await page.extract({
      instruction: prompt_extract2 + terms,
      schema: ret
      //modelName: model
    });
  }
...
function splitWithOverlap(data: string, chunkSize = 3000, overlap = 20): string[] {
  const chunks: string[] = [];
  let start = 0;
